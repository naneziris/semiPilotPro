...
├── agents/                     # 5 agents (was 9)
│   ├── refiner.agent.md
│   ├── planner.agent.md
│   ├── spec-critic.agent.md    # Gate 1
│   ├── pattern-critic.agent.md # Gate 2
│   └── scribe.agent.md         # wiki & docs updater
...
- `manager` agent (Dev drives the pipeline directly — no orchestrator needed)
- `registry` agent (routing is the pipeline itself)
...
Exactly 2 human sync points per successful cycle. On rejection, Dev decides to retry or abandon.
...
4. Kick off a real task with `/refine-requirements`.
