...
- `/code-analyzer` for complexity checks (run `python skills/code-analyzer/run.py <file>` or invoke `/code-analyzer`)
...
5. **Complexity.** Run `/code-analyzer` on each changed file. Any function exceeding cyclomatic complexity 15 without a `# complexity-exempt: <reason>` comment → REJECT. File-level complexity increased more than 20% → FLAG (not reject), attach to verdict.
