...
- **Full orchestration:** `@manager "I want to add X"` — the manager runs the whole pipeline and prompts you at the three sync gates.
- **Manual driving:** run each prompt yourself (`/refine-requirements`, then `/create-implementation-plan`, then `/implement-plan`) and invoke the critics by hand. Slower but more transparent.
...
This follows the **YAML rail** (`copilot-instructions.md`) step-by-step:
...
5. lint
6. unit_tests
7. complexity_check (runs `#code-analyzer`, threshold 15)
8. wiki_pattern_check
9. submit → @pattern-critic

The implementer reports every step. Skipping is detected and rejected by the manager.
...
Reads the diff against `.wiki/PATTERNS.md`, `DEPENDENCIES.md`, and `API.md` plus runs `#code-analyzer` on changed files. Binary verdict.

- **APPROVED** → pause for your approval, then continue.
- **REJECTED** → the critic lists the specific fixes in order. `/implement-plan` re-runs until the diff passes.
...
## 3. The three human sync points

The whole pipeline pauses for your explicit approval exactly three times:
...
| **Reject gate** | Any time a critic REJECTS | Retry with the critic's fix, or abandon? |

No other automatic pauses. Internal handoffs are automatic — you don't sign off between refiner and planner.

---

## 4. Using components on their own

Everything in SemiPilot Pro is designed to be useful in isolation. Here's when to reach for each piece without the full pipeline.
...
| `@manager` | Only useful for the full pipeline. Skip for any partial flow. |
...
| `/implement-plan` | You already have a plan and want code written under the YAML rail — tests first, linted, complexity-checked. |
...
Once the rule is in the wiki, `@pattern-critic` will block any future diff that repeats the mistake. You can run `@pattern-critic` on any diff independently — without the full pipeline — to verify the new rule catches what you expect.

**"I want to improve the system itself."**
Treat semiPilot Pro as the codebase being modified. For non-trivial changes (a new gate, a changed rail step, a new agent), run the full pipeline — `/refine-requirements` describes the improvement, and the critics validate the change against the system's own architecture.

For smaller improvements (tightening a convention, banning a library, adjusting a complexity threshold): update the relevant wiki file directly, then hand the change to `@scribe` to log it in `ARCH_DECISIONS.md`.
...
- **Center column** = the pipeline, top to bottom.
- **Yellow sync bars** = the three places you are expected to respond.
...
- **YAML rail callout** (bottom-left) = the deterministic sequence `/implement-plan` must follow.
...
The critic only enforces what's in `.wiki/PATTERNS.md`. Remove the convention from the wiki, or change it there — don't argue with the critic.
...
Either amend the wiki (by opening a plan that updates it — critics can authorize their own rules changing) or argue the case with Dev first. Don't silently bypass.

**"The diagram says `@manager` but I'd rather run prompts manually."**
Fine. The manager is convenience, not requirement. Run `/refine-requirements` yourself, then `@spec-critic`, then the rest by hand. Same pipeline, more visibility.
...
- No agent-spawns-agent ceremony. Six agents, three gates, one rail.
...
The user flow should live in `.wiki/OVERVIEW.md` or be derivable from the wiki. Wiki-only is enough:
...
Authentication spans both documented architecture (wiki) and the actual code path (source). Use `--include-source`:
...
This is entirely about component-level source code, not wiki-level architecture. Two options depending on project size:
...
This requires source. Scope it to the relevant directory if you can — it reduces noise and avoids truncation:
...
Writes the failing test first (red), then makes it pass (green), lints, runs the full suite. The YAML rail applies exactly as for a feature — tests first, no exceptions.
...
**Step 6 — `@scribe`**
If the bug revealed an undocumented edge case, invariant, or constraint, `@scribe` adds it to the wiki so the critics can catch it in future. Example: *"The bug revealed that `TransactionService` must never be called outside a database transaction. Add that to `PATTERNS.md`."*
...
Add a row to the Agent Inventory table in [`copilot-instructions.md`](copilot-instructions.md) under the correct model. This is how `@manager` knows the agent exists and how humans discover it.
