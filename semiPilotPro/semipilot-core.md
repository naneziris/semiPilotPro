...
GitHub Copilot in VS Code currently ignores the `model:` field in a subagent's own `.agent.md` frontmatter — the spawned subagent inherits the caller's model unless the caller overrides it at spawn time. The `@manager` works around this by naming the model explicitly in every spawn (`Spawn @<agent> with model <Model Name (copilot)> and task: …`). The model column above is the source of truth for those overrides; see `agents/manager.agent.md` → "Subagent Model Selection" for the exact spawn strings.
