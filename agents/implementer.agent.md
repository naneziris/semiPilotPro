...
3. Read the plan in full. Read `.wiki/PATTERNS.md` in full. Cache the patterns content — you will use it again in Step 10 without re-reading the file.
...
## Step 6: type_check
- TypeScript: run `tsc --noEmit` (or the equivalent script in `package.json`). Python: run `mypy <src dir>`.
- Fix every type error. Do not suppress errors with `// @ts-ignore`, `// @ts-expect-error`, `# type: ignore`, or any equivalent unless the plan explicitly authorizes it and explains why.
...
- For each modified test file, produce an entry in this format:
  ```
  Modified test: <path>
  Reason: <why this existing test needed to change — cite the specific implementation step or pattern change>
  ```
- **Block:** if any pre-existing test file was modified and you cannot name a specific reason from the plan or a wiki pattern change.
...
- Using the `PATTERNS.md` content already loaded in Step 2 — do not re-read the file.
- For each file you changed, verify naming, DI, error handling, and test style match the cached patterns.
- **Block:** on any deviation you cannot justify by citing a specific pattern.
