...
- The codebase, via `search` and `usages`, for **impact analysis** (see Process step 3).
...
4. **Decomposition check.** Apply the trigger conditions in `semipilot-core.md > Decomposition Policy`:
   ...
   If **any** condition holds, decompose: produce `requirements-index.md` + per-sub-requirement files using the index schema in `semipilot-core.md`. Each sub-file follows the single-requirement structure above and must include its own Impact Analysis scoped to that sub-change.
