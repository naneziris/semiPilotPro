...
3. Read the plan in full. Read the conventions corpus in full: `.github/copilot-instructions.md` plus the `.github/instructions/*.instructions.md` files whose `applyTo` globs match the files in play for the plan's Files to Change, plus the cards listed in the plan's Knowledge References.
...
- Read the conventions corpus completely: `.github/copilot-instructions.md` plus the `.github/instructions/*.instructions.md` files whose `applyTo` globs match the files in play for the plan's Files to Change, plus the `invariants:` of the cards in the plan's Knowledge References.
...
- **Hard block:** if any lint error remains after your fix attempt, stop here. Post the first 50 lines of error output (append `[truncated — N lines total]` if there is more). Do not proceed to step 6.
...
- **Hard block:** if any type error remains after your fix attempt, stop here. Post the first 50 lines of `tsc` / `mypy` output (append `[truncated — N lines total]` if there is more). Do not proceed to step 7.
...
- **Hard block:** if any test fails, stop immediately. Post:
  ...
  2. The first 50 lines of failure output (append `[truncated — N lines total]` if there is more).
...
- For each modified test file, produce an entry in this format:
  ...
  ```
- **Block:** if any pre-existing test file was modified and you cannot name a specific reason from the plan or a documented convention change, stop and report: "Modified test `<path>` has no documented reason. Explain why this test was changed before proceeding."
...
- **Do not write to `docs/cards/` or the other knowledge docs.** That is `@scribe`'s job.
