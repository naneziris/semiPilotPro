...
You are writing unit tests for an existing source file. The code already exists — you are not building a new feature. Your job is to cover observable behavior, not implementation details. You must follow the test conventions in `.github/instructions/tests.instructions.md` (plus `.github/copilot-instructions.md`) exactly.

Take a deep breath and work through this step by step.

# PRE-FLIGHT

1. Confirm `.github/instructions/tests.instructions.md` exists and is non-empty. If it is empty or missing, stop and report: "Test conventions not populated. Fill in `.github/instructions/tests.instructions.md` before running `/write-tests`."
...
Read the conventions corpus completely (`.github/copilot-instructions.md` plus the `.github/instructions/*.instructions.md` files whose `applyTo` globs match the files in play). Extract and note:
...
Place the file at the path dictated by the repo's test conventions (see the tests instructions file).
...
- **Do not write to `docs/cards/` or the instructions files.** If new conventions emerge from this work, note them in the report; `@scribe` will record them after `@pattern-critic` approves.
...
- **Follow the instructions files exactly.** If your instinct conflicts with a documented convention, the convention wins.
